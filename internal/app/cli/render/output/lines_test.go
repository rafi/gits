...
func TestLinesRendersBodyOrBareError(t *testing.T) {
	...
	res := command.Results[string]{Project: proj, Results: []command.Result[string]{
		{Repo: command.Repo{Repository: proj.Repos[0], Project: proj, Path: "api"},
			Value: "[main <- origin/main] ok"},
		{Repo: command.Repo{Repository: proj.Repos[1], Project: proj, Path: "web"},
...
func TestLinesSeparatesProjects(t *testing.T) {
	...
	line := func(p domain.Project, key string, i int) command.Result[string] {
	...
	res := command.Results[string]{Project: root, Results: []command.Result[string]{
...
func TestErrorsWrapsForTheEpilogue(t *testing.T) {
	...
	res := command.Results[string]{
		Results: []command.Result[string]{
			{Repo: command.Repo{Repository: repo}, Err: errors.New("boom")},
			{Repo: command.Repo{Repository: repo}},
			{Repo: command.Repo{Repository: repo}, Err: warning},
		},
...
func TestLinesGroupsProjectsCopiedThroughAppend(t *testing.T) {
	...
	res := command.Results[string]{Project: root, Results: []command.Result[string]{
		{Repo: command.Repo{Repository: root.Repos[0], Project: root, ProjectKey: "0", Path: "a"},
			Value: "BODY"},
		{Repo: command.Repo{Repository: grown.Repos[1], Project: grown, ProjectKey: "0", Path: "b"},
			Value: "BODY"},
		{Repo: command.Repo{Repository: sub.Repos[0], Project: sub, ProjectKey: "0/0", Path: "s"},
			Value: "BODY"},
	}}
...
func jsonFixture(t *testing.T) (command.Results[string], *clitest.Deps) {
	t.Helper()
	...
	repo := func(i int) command.Repo {
		return command.Repo{Repository: proj.Repos[i], Project: proj, ProjectKey: "0", Path: proj.Repos[i].Dir}
	}
	res := command.Results[string]{Command: "pull", Project: proj, Results: []command.Result[string]{
		...
		{Repo: repo(3), Err: command.StateError(proj.Repos[3]), Guarded: true},
	}}
	return res, deps
}
...
func TestJSONSkippedProject(t *testing.T) {
	...
	deps := clitest.New(t, nil)
	if err := JSON(command.Results[string]{Command: "clone"}, textView, deps.RuntimeCLI); err != nil {
...
func TestJSONTree(t *testing.T) {
	...
	res := command.Results[string]{Command: "fetch", Project: root, Results: []command.Result[string]{
		// Reversed from traversal order on purpose.
		{Repo: command.Repo{Repository: sub.Repos[0], Project: sub, ProjectKey: "0/0"}, Value: "sub"},
		{Repo: command.Repo{Repository: root.Repos[0], Project: root, ProjectKey: "0"}, Value: "root"},
	}}
