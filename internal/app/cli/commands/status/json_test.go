...
func row(repo domain.Repository, st status.Report) *status.Report {
	st.Repo = command.Repo{Repository: repo, Path: repo.Name}
	return &st
}

// results wraps statuses as the run hands them to the renderer, under the
// tree the run visited.
func results(project domain.Project, sts ...*status.Report) command.Results[*status.Report] {
	res := command.Results[*status.Report]{Project: project}
	for _, st := range sts {
...
func renderDoc(t *testing.T, res command.Results[*status.Report], opts Options) map[string]any {
	t.Helper()
...
func TestStatusJSONHead(t *testing.T) {
	...
	st, err := status.Probe{}.Repo(t.Context(), command.Repo{
