...
func TestExecPullSingleRepo(t *testing.T) {
	...
	if err := ExecPull("table", domain.TagSet{}, []string{"acme", "api"}, deps.RuntimeCLI); err != nil {
		t.Fatalf("ExecPull error = %v, want nil", err)
	}
...
func TestExecPullSkipsNonOKRepositories(t *testing.T) {
	...
	err := ExecPull("table", domain.TagSet{}, []string{"acme"}, deps.RuntimeCLI)
	if err == nil {
...
func TestExecPullUnpullableIsSkipped(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := ExecPull("table", domain.TagSet{}, []string{"acme", "api"}, single.RuntimeCLI); err != nil {
...
func TestExecPullHeadFailure(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := ExecPull("table", domain.TagSet{}, []string{"acme", "api"}, deps.RuntimeCLI)
			if err == nil {
...
func TestExecPullJSON(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := ExecPull("json", domain.TagSet{}, []string{"acme"}, deps.RuntimeCLI); err != nil {
...
func TestExecPullJSONInterrupted(t *testing.T) {
	...
	err := ExecPull("json", domain.TagSet{}, []string{"acme"}, deps.RuntimeCLI)
...
func TestExecPullUnknownFormat(t *testing.T) {
	...
	err := ExecPull("wide", domain.TagSet{}, []string{"acme"}, deps.RuntimeCLI)
