// Package cli assembles the gits command tree: a fast CLI Git manager for
// multiple repositories grouped by projects, with GitHub, GitLab and
// Bitbucket support. [Main] is the whole binary; cmd/gits only calls it.
package cli
...
var (
	...
	// handed to every command on coreruntime.Runtime. It is nil until cobra's
...
// Main runs the command tree and exits non-zero on failure. It is the
// binary: cmd/gits/main.go calls it and nothing else.
func Main() {
	registerRootFlags()
	...
	cobra.OnInitialize(func() {
		...
		// returns nil, so git-free commands (add, version) still command.
...
// stays silent.
func newRuntime(ctx context.Context) (coreruntime.Runtime, []error) {
