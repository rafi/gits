...
func TestNewNonTTYReturnsNop(t *testing.T) {
	...
	for _, w := range []io.Writer{io.Discard, &bytes.Buffer{}} {
		if _, ok := New(w).(command.Nop); !ok {
			t.Errorf("New(%T) = %T, want command.Nop", w, New(w))
		}
	}
}
...
func TestNopLifecycle(t *testing.T) {
	...
	const total = 5
	var r command.Progress = command.Nop{}
...
func TestReporterSatisfiedByImpls(t *testing.T) {
	...
	var _ command.Progress = command.Nop{}
	var _ command.Progress = (*liveReporter)(nil)
}
