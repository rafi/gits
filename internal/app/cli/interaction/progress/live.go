...
const (
	// roundHalf is added before truncation to round to the nearest whole.
	roundHalf = 0.5
	// percentScale turns a 0..1 ratio into whole percent.
	percentScale = 100
	// statsPrecision is how finely the elapsed time in the summary is shown.
	statsPrecision = 100 * time.Millisecond
)
...
func New(w io.Writer) command.Progress {
...
func (r *liveReporter) totalStats(done, total int, start time.Time) string {
	return r.st.stats.Render(fmt.Sprintf("%d/%d · %s",
		done, total, time.Since(start).Round(statsPrecision)))
}
